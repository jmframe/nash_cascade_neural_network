...
class NashCascadeNetwork(nn.Module):
    ...
    def initialize(self):
        ...
        if self.do_predict_theta_with_lstm:
            self.lstm_hidden_size = 64  # Set according to your needs
...
#            zeros_tensor = torch.zeros((n_buckets, n_spigots), requires_grad=True)
    ...
    def solve_buckets_batch(self, H, S, theta_indices, bucket_inflows):
        ...
        if torch.any(H_final < -0.001):
            print("---------------WARNING---------------")
            print("H_final contains values less than -0.001")
            print("H_final is leq zero", H_final)
            print("s_q_local is", s_q_local)
...
#        self.theta = self.theta.detach()
    ...
    def check_gradients(self, print_all_gradients=False):
        if print_all_gradients:
            print("Here are all the lstm gradients:")
            print(self.lstm_output.grad)
        if self.lstm_output.grad is not None:
            # Find non-zero gradients
            non_zero_grads = self.lstm_output.grad != 0
...
def train_model(model, u, y_true):
    ...
    optimizer = optim.Adam(all_params, lr=model.train_learning_rate)
